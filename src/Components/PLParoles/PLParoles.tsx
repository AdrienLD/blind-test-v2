...
const transformString = (input: string | undefined | null): string =>
  (input ?? '')
    .split('')
    .map(char => {
      if (char === ' ') return ' '
      return /[a-zA-ZêôÔçàéèùûïîëüÉÈÀÇÊÎÔÛ]/i.test(char) ? '_' : char
    })
    .join('')
...
function PLParoles() {
  ...
  const isValidLyricLine = (index:number, musique: MusicWLyrics) => {
    const line = musique.lyrics[index]
    if (!line || !line.words || line.words.trim() === '') return false
    if (line.words.startsWith('(') && line.words.endsWith(')')) return false
    for (let offset = 1; offset <= 3; offset++) {
      const prev = musique.lyrics[index - offset]
      if (prev && prev.words === line.words) return false
    }
    return true
  }

  const getRandomStartTime = (musique: MusicWLyrics ) => {
    const total = musique.lyrics.length
    if (total === 0) return 0 
    let attempts = 0
    const maxAttempts = 100
    let indexToGuess = Math.floor(Math.random() * total)
    while (!isValidLyricLine(indexToGuess, musique) && attempts < maxAttempts) {
      indexToGuess = Math.floor(Math.random() * total)
      attempts++
    }

    const startIndex = Math.max(0, indexToGuess - 5)
    console.log('getRandomStartTime', startIndex, musique.lyrics[startIndex], musique.lyrics[indexToGuess])
    return startIndex
  }
